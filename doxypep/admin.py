...
class SequenceStatusFilter(admin.SimpleListFilter):
    ...
    parameter_name = 'need_resequencing'  # also fix the space here
    ...
    def queryset(self, request, queryset):
        if self.value() == 'yes':
            return queryset.filter(need_resequencing=True)  # was: status=True
        if self.value() == 'no':
            return queryset.filter(need_resequencing=False)  # was: status=False
        return queryset

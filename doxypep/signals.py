...
def set_isolate_position(sender, instance, **kwargs):
    ...
    if not last_record:
        ...
        try:
            # Case-insensitive replacement ensures the conversion doesn't fail on mixed case letters
            cleaned_box = last_record.box.upper().replace('ZUC_DOXY_ISO_B', '')
...
'''
@receiver(pre_save, sender=Kb)
def autofill_participant_info(sender, instance, **kwargs):
    if not instance.participant_id_id or not instance.visit_num or not instance.sample_type_id:
        isolate = instance.isolate_num
        instance.participant_id = isolate.participant_id
        instance.visit_num = isolate.visit_num
        instance.sample_type = isolate.sample_type
'''